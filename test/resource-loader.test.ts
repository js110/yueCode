...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "yue-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".yue", "prompts");
			...
			const projectSkillDir = join(cwd, ".yue", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".yue", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".yue", "themes"), { recursive: true });
		...
		it("should keep both extensions loaded when command names collide", async () => {
			...
			const projectExtDir = join(cwd, ".yue", "extensions");
			mkdirSync(userExtDir, { recursive: true });
			mkdirSync(projectExtDir, { recursive: true });

			writeFileSync(
				join(projectExtDir, "project.ts"),
				`export default function(yue) {
	yue.registerCommand("deploy", {
		description: "project deploy",
		handler: async () => {},
	});
	yue.registerCommand("project-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.yue", async () => {
			const yueDir = join(cwd, ".yue");
			mkdirSync(yueDir, { recursive: true });
			writeFileSync(join(yueDir, "SYSTEM.md"), "You are a helpful assistant.");
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const yueDir = join(cwd, ".yue");
			mkdirSync(yueDir, { recursive: true });
			writeFileSync(join(yueDir, "APPEND_SYSTEM.md"), "Additional instructions.");
	...
	describe("extension conflict detection", () => {
		it("should detect tool conflicts between extensions", async () => {
			...
			writeFileSync(
				...
				`
import type { ExtensionAPI } from "yue";
import { Type } from "@sinclair/typebox";
export default function(yue: ExtensionAPI) {
  yue.registerTool({
    name: "duplicate-tool",
    description: "First",
