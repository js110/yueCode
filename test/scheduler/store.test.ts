...
describe("scheduler store atomic writes", () => {
	const originalEnv = process.env.YUE_HOME;
	let home: string;

	beforeEach(() => {
		home = mkdtempSync(join(tmpdir(), "yue-scheduler-store-"));
		process.env.YUE_HOME = home;
	});

	afterEach(() => {
		if (originalEnv === undefined) delete process.env.YUE_HOME;
		else process.env.YUE_HOME = originalEnv;
		rmSync(home, { recursive: true, force: true });
	});

	it("replaces an existing tasks.json atomically", async () => {
		// Re-import after YUE_HOME is set so getSchedulerDir() picks it up.
