...
beforeEach(() => {
	originalHome = process.env.YUE_HOME;
	home = join(tmpdir(), `yue-sched-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	mkdirSync(home, { recursive: true });
	process.env.YUE_HOME = home;
});

afterEach(() => {
	if (originalHome === undefined) delete process.env.YUE_HOME;
	else process.env.YUE_HOME = originalHome;
