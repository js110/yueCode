...
describe("main print facade resume route", () => {
	...
	const originalOffline = process.env.YUE_OFFLINE;
	...
	afterEach(() => {
		...
		if (originalOffline === undefined) {
			...
			process.env.YUE_OFFLINE = originalOffline;
		}
	...
	function makeTempDir(): string {
		const dir = join(tmpdir(), `yue-main-print-facade-resume-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	): (yue: ExtensionAPI) => void {
		return (yue: ExtensionAPI) => {
			yue.registerProvider("facade-test", {
