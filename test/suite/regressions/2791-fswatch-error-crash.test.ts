...
/**
 * Regression test for https://github.com/badlogic/yue/issues/2791
 *
...
describe.skip("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "yue-2791-"));
...
process.env.YUE_CODING_AGENT_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync("npx", ["tsx", scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, YUE_CODING_AGENT_DIR: agentDir },
