...
describe("Coding Agent Tools", () => {
	...
	describe("cli tool", () => {
		...
		it("should prefer bundled fd/rg binaries from the package", async () => {
			const originalPackageDir = process.env.YUE_PACKAGE_DIR;
			...
			try {
				process.env.YUE_PACKAGE_DIR = packageDir;
				expect(getToolPath("rg")).toBe(bundledRgPath);
			} finally {
				if (originalPackageDir === undefined) {
					delete process.env.YUE_PACKAGE_DIR;
				} else {
					process.env.YUE_PACKAGE_DIR = originalPackageDir;
				}
			}
		});
