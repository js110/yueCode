...
const skipInOfflineMode = process.env.YUE_OFFLINE === "1";
...
let tempDir: string;
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const cachedDir = join(tmpdir(), "yue-extensions", `git-${gitHost}`, hash, gitPath);
			const extensionFile = join(cachedDir, "yue-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "yue-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ yue: { extensions: ["./yue-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "yue-extensions/session-breakdown.ts")).toBe("// fresh");
		});
		...
		it("should not refresh pinned temporary git sources", async () => {
			...
			expect(getFileContent(cachedDir, "yue-extensions/session-breakdown.ts")).toBe("// pinned");
		});
	});
	...
	describe("scope-aware update", () => {
		it("should not install locally when source is only registered globally", async () => {
			...
			const projectGitDir = join(tempDir, ".yue", "git", "github.com", "test", "extension");
