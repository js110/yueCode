...
describe("BrandHeaderComponent", () => {
	...
	it("renders the Yue brand and startup value proposition", () => {
		const header = new BrandHeaderComponent("yue", "1.2.3");
		const rendered = stripAnsi(header.render(90).join("\n"));

		expect(rendered).toContain("YUE");
		expect(rendered).toContain("v1.2.3");
		expect(rendered).toContain("Create together with Yue.");
	...
	it("keeps all compact and expanded lines within render width", () => {
		for (const width of [20, 45, 46, 68, 90]) {
			const header = new BrandHeaderComponent("yue", "1.2.3", true);
