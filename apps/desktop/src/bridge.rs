...
/// Read an API key for a provider from ~/.yue/agent/auth.json.
...
fn find_node() -> Option<String> {
	// Check YUE_NODE env var first.
	if let Ok(node) = std::env::var("YUE_NODE") {
...
fn resolve_yue_command(app: &AppHandle) -> (String, Vec<String>) {
	// 1. YUE_BIN env var — explicit override (any executable, including a
	//    manually-built `dist/yue` Bun binary).
	if let Ok(bin) = std::env::var("YUE_BIN") {
	...
	//    target/debug/ where a stale dist/yue may exist from a prior
	//    `build:binary`, which can hang the sidecar.
	if !cfg!(debug_assertions) {
		if let Ok(resource_dir) = app.path().resource_dir() {
			let yue_bin = resource_dir.join(if cfg!(windows) { "yue.exe" } else { "yue" });
			if yue_bin.exists() {
				log_file(&format!(
					"resolve_yue_command: using bundled binary at {}",
					yue_bin.display()
				));
				return (
					yue_bin.to_string_lossy().to_string(),
					vec!["--mode".to_string(), "rpc".to_string()],
				);
			}
		}
	...
	log_file(&format!(
		"resolve_yue_command: dev fallback, node={}, cli_js={}, loader={}",
		node,
		cli_js.display(),
		loader.display()
	));
...
///
/// Provider credentials live in the shared `~/.yue/agent/auth.json`, which is
...
fn workspace_meta_root() -> Option<PathBuf> {
	home_dir().map(|home| home.join(".yue").join("agent").join("workspaces"))
}
...
fn spawn_background_sidecar(
	...
	let (program, mut args) = resolve_yue_command(&app);
	if is_persistent_chat_cwd(&cwd) {
...
/// `cwd` is the working directory for the yue rpc process (the user's project).
#[tauri::command]
pub async fn init_sidecar(
	...
	// The persistent Chat workspace (~/.yue/main) is auto-created on first
	...
	let is_persistent_chat = std::env::var("HOME")
		.ok()
		.map(|home| cwd == format!("{}/.yue/main", home))
		.unwrap_or(false);
	...
	let (program, mut args) = resolve_yue_command(window.app_handle());
	// When the workspace is the persistent Chat (~/.yue/main), launch yue
...
pub async fn restart_sidecar(
	...
	// Drop stdin + reap child so the OS releases the .lock in ~/.yue/main
...
/// List all workspaces from ~/.yue/agent/workspaces/*/meta.json
#[tauri::command]
...
/// be absolute — the sidecar places uploads under <cwd>/.yue/uploads/...
...
///
/// Search order mirrors `resolve_yue_command`:
...
pub async fn set_provider_api_key(
	...
	let home = std::env::var("HOME").map_err(|_| "HOME not set")?;
	let auth_dir = PathBuf::from(&home).join(".yue").join("agent");
...
/// The OpenAI API key is read from ~/.yue/agent/auth.json under the "openai" key.
#[tauri::command]
