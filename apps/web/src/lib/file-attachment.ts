 ...
 * - non-image files → uploaded to <cwd>/.yue/uploads/<ws>/<session>/<uuid>-<name>
 ...
 * wrote the original bytes to <cwd>/.yue/uploads/<ws>/<session>/<uuid>-<name>;
...
export async function loadFileAttachment(file: File): Promise<AnyAttachment> {
	...
	// Anything else: upload to <cwd>/.yue/uploads/<ws>/<session>/<uuid>-<name>.
