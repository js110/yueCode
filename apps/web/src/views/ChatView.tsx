	...
	for (const msg of messages) {
		...
		if (role === "user") {
			...
			if (text || thinking || images.length > 0) {
				history.push({ id: `hist-${history.length}`, role: "assistant", title: t("common.yue"), text, status: "DONE", streaming: false, thinking: thinking || undefined, images: images.length > 0 ? images : undefined, timestamp: ts });
			}
	...
	const handleEvent = useCallback((event: TypedEvent & { _cwd?: string }) => {
		...
		switch (event.type) {
			...
			case "AGENT_MESSAGE_START": {
				...
				updateItems((prev) => [
					...prev,
					{ id, role: "assistant", title: tRef.current("common.yue"), text: "", status: "STREAMING", streaming: true, timestamp: ts },
				]);
				break;
			}
