...
function AppInner() {
	...
	const refreshWorkspaces = useCallback(async () => {
		...
		try {
			...
			// Filter out the Chat workspace (~/.yue/main) — it's shown separately as Chat.
			setWorkspaces(list.filter((ws) => !ws.cwd.endsWith("/.yue/main")));
		} catch (e) {
	...
	useEffect(() => {
		...
		if (isTauri()) {
			// In Tauri: auto-start with Chat (persistent agent at ~/.yue/main)
			startWithWorkspace("~/.yue/main");
		} else {
			...
			<Routes>
				...
				onRefreshState={refreshState}
					...
					<Route path="/schedules" element={
						<SchedulesView
							scope={workspace && workspace.endsWith("/.yue/main") ? "main" : "workspace"}
