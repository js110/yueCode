...
function authPath() {
	return path.join(os.homedir(), ".yue", "agent", "auth.json");
}
...
/**
 * Vite dev plugin: spawns `yue rpc` as a child process and exposes
 * HTTP endpoints for the browser to communicate with it.
 *
 *   POST /rpc/command   → send a JSON command to yue stdin
...
export function yueRpcBridge() {
	...
	function ensureChild() {
		if (child) return;
		console.log("[yue-rpc-bridge] spawning yue --mode rpc...");
		child = spawn("yue", ["--mode", "rpc"], {
		...
		child.stderr.on("data", (chunk) => {
			...
			if (text) console.error(`[yue-rpc-bridge stderr] ${text}`);
		});

		child.on("error", (err) => {
			console.error(`[yue-rpc-bridge] spawn error: ${err.message}`);
			child = null;
		});

		child.on("exit", (code, signal) => {
			console.log(`[yue-rpc-bridge] child exited code=${code} signal=${signal}`);
			child = null;
		});
	}
	...
	return {
		...
		configureServer(server) {
			...
			if (!child?.stdin?.writable) {
				res.statusCode = 503;
				res.setHeader("Content-Type", "application/json");
				res.end(JSON.stringify({ error: "yue not running" }));
				return;
			}
				...
				req.on("end", () => {
					try {
						...
						if (child?.stdin?.writable) {
							...
							res.statusCode = 503;
							res.end(JSON.stringify({ error: "yue process not running" }));
						}
					} catch (e) {
						res.statusCode = 400;
