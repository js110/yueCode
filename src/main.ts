...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.YUE_OFFLINE);
	if (offlineMode) {
		process.env.YUE_OFFLINE = "1";
		process.env.YUE_SKIP_VERSION_CHECK = "1";
	}
	...
	if (appMode === "rpc") {
		...
		const startupBenchmark = isTruthyEnvFlag(process.env.YUE_STARTUP_BENCHMARK);
		if (startupBenchmark) {
			console.error(chalk.red("Error: YUE_STARTUP_BENCHMARK only supports interactive mode"));
			process.exit(1);
		}
