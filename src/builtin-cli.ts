/**
 * `yue builtin` — manage built-in extensions from the CLI.
 *
 * Built-in extensions live inside Yue and are loaded by the resource loader.
 ...
 * Usage:
 *   yue builtin list
 *   yue builtin enable <id>
 *   yue builtin disable <id>
 */
...
export async function handleBuiltinCommand(args: string[]): Promise<boolean> {
	...
	switch (subcommand) {
		...
		case "enable": {
			if (!id) {
				console.error(chalk.red("Missing extension id. Usage: yue builtin enable <id>"));
				process.exitCode = 1;
				return true;
			}
		...
		case "disable": {
			if (!id) {
				console.error(chalk.red("Missing extension id. Usage: yue builtin disable <id>"));
				process.exitCode = 1;
				return true;
			}
		...
		default:
			console.log(`Usage:
  yue builtin list              List built-in extensions and their state
  yue builtin enable <id>       Enable a built-in extension
  yue builtin disable <id>      Disable a built-in extension`);
			return true;
	}
}
