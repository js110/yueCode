 ...
 *
 * Scopes:
 *   - "main"      → ~/.yue/main/scheduler/{tasks.json,runs.jsonl}
 *   - "workspace" → ~/.yue/workspaces/<workspaceId>/scheduler/{tasks.json,runs.jsonl}
 *
...
import type { ScheduledTask, ScheduledTaskRun } from "@yue/protocol";
...
function getSchedulerDir(scope: "main" | "workspace", workspaceId?: string): string {
	if (scope === "main") {
		...
		const home = process.env.YUE_HOME ?? join(
