...
import { injectYuePathShims } from "../../utils/path-shims.js";
import {
	getShellConfig,
	getShellEnv,
	killProcessTree,
...
function getTempFilePath(): string {
	...
	return join(tmpdir(), `yue-bash-${id}.log`);
}
...
/**
 * Create bash operations using Yue's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want Yue's
	...
	return {
		exec: (command, cwd, { onData, signal, timeout, env }) => {
			return new Promise((resolve, reject) => {
				...
				const childEnv = injectYuePathShims(env ?? getShellEnv());
...
/**
 * Detect a Yue built-in command name that appears as the FIRST word of a
				...
				ops.exec(spawnContext.command, spawnContext.cwd, {
					...
					.then(({ exitCode }) => {
						...
						if (chainedBuiltin) {
							outputText +=
								"\n\n" +
								`(${chainedBuiltin} is a Yue built-in cli command. It cannot be ` +
