...
export interface CreateSessionFacadeOptions {
	...
	cwd?: string;
	/** Global config directory. Default: ~/.yue/agent */
	agentDir?: string;
	...
	extensionRunner.bindCore(
		{
			...
			setModel: async (nextModel) => {
				...
				try {
					...
					console.warn(
						`[yue] failed to persist model preference (${nextModel.provider}/${nextModel.id}): ${e instanceof Error ? e.message : String(e)}`,
					);
				}
				return true;
			},
			...
			setThinkingLevel: (level) => {
				...
				if (isPersistableThinkingLevel(level)) {
					try {
						...
						console.warn(
							`[yue] failed to persist thinking-level preference (${level}): ${e instanceof Error ? e.message : String(e)}`,
						);
					}
				}
			},
