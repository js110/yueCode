...
// avoiding a circular dependency. Extensions can import from "yue".
import * as _bundledYue from "../../index.js";
...
function readYueManifest(packageJsonPath: string): YueManifest | null {
	try {
		const content = fs.readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content);
		if (pkg.yue && typeof pkg.yue === "object") {
			return pkg.yue as YueManifest;
		}
		return null;
	} catch {
		return null;
 ...
 *
 * Checks for:
 * 1. package.json with "yue.extensions" field -> returns declared paths
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readYueManifest(packageJsonPath);
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "yue" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with yue manifest or index.ts
