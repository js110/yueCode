...
import { SCHEDULED_TASK_FIRED, SCHEDULED_TASK_COMPLETED, type ScheduledTaskPatch, type SessionTarget } from "@yue/protocol";
 ...
 * under <cwd>/.yue/uploads/<ws>/<session>/; we only validate structurally here.
 */
...
export async function runRpcModeWithFacade(facade: SessionFacade): Promise<never> {
	...
	// ~/.yue/main/scheduler/tasks.json (or per-workspace when the sidecar
	...
	const handleCommand = async (command: RpcCommand): Promise<RpcResponse | undefined> => {
		...
		switch (command.type) {
			...
			case "set_scheduler_policy": {
				const policy = (command as unknown as { policy: import("@yue/protocol").SchedulerPolicy }).policy;
		...
		case "set_extension_enabled": {
			...
			if (!info) {
				return error(
					...
					"Only built-in extensions can be toggled. Manage other extensions via `yue plugin`.",
				);
			}
