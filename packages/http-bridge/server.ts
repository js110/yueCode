...
export async function runGuiModeWithFacade(facade: SessionFacade, options: GuiModeOptions): Promise<never> {
	...
	console.log(`Yue GUI listening on ${url}`);
...
<body>
	<div id="app" class="app">
		...
		<footer class="composer">
			<form id="composerForm" class="composer-inner">
				<textarea id="promptInput" placeholder="Ask Yue to work on this project"></textarea>
	...
	<script>
		...
		function titleForMessage(message) {
			...
			if (message.role === "assistant") return "Yue";
		...
		function render() {
			if (state.items.length === 0) {
				timeline.innerHTML =
					'<div class="empty"><div class="empty-title">Yue GUI</div><div>Current project is ready.</div></div>';
				renderMeta();
				return;
			}
