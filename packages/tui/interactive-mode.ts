...
export class InteractiveMode {
	...
	private updateTerminalTitle(): void {
		...
		this.ui.terminal.setTitle(`Yue - ${cwdBasename}`);
	}
	...
	private async checkTmuxKeyboardSetup(): Promise<string | undefined> {
		...
		if (extendedKeysFormat === "xterm") {
			return "tmux extended-keys-format is xterm. Yue works best with csi-u. Add `set -g extended-keys-format csi-u` to ~/.tmux.conf and restart tmux.";
		}
	...
	private async handleClipboardImagePaste(): Promise<void> {
		try {
			...
			const fileName = `yue-clipboard-${crypto.randomUUID()}.${ext}`;
	...
	private openExternalEditor(): void {
		...
		const tmpFile = path.join(os.tmpdir(), `yue-editor-${Date.now()}.yue.md`);
