...
function selectedTargets() {
	const value = process.env.YUE_VENDOR_TOOL_TARGETS;
...
async function download(url) {
	if (process.env.YUE_VENDOR_TOOL_DOWNLOAD === "gh") {
...
async function downloadWithGh(url) {
	...
	const tempDir = await mkdtemp(path.join(tmpdir(), "yue-gh-download-"));
	try {
...
async function installTool(targetName, toolName, asset) {
	const tempDir = await mkdtemp(path.join(tmpdir(), `yue-${toolName}-${targetName}-`));
	try {
...
async function main() {
	if (process.env.YUE_SKIP_VENDOR_TOOLS === "1") {
		console.log("Skipping bundled fd/rg download because YUE_SKIP_VENDOR_TOOLS=1");
		return;
	}
