# 持久化 Agent 设计文档（`~/.yue/main`）

## 1. 背景与动机

当前 Yue 的所有 workspace 被一视同仁，差异只来自 `cwd`：
...
`~/.yue/main` 要成为一个**常驻人格 agent**（类似 OpenClaw 的持久化 agent），与普通 per-project workspace 区别对待：拥有灵魂文件、长期记忆。

**单实例，可选常驻**：main agent 全局只允许一个实例运行。既支持前台交互模式（`yue --main`，用完即退），也支持常驻后台模式（`yue --main --daemon`，进程持续运行，多个用户会话在同一进程内交替创建和结束）。两种模式下记忆和灵魂机制完全一致，区别仅在于进程生命周期。

## 2. 设计目标

| 维度 | 普通 workspace | `~/.yue/main` |
|---|---|---|
| 身份 | 无（通用 coding assistant） | 灵魂文件定义人格 |
| 记忆 | 仅当次会话 + AGENTS.md | 长期记忆库|
| 工作范围 | 绑定单个 cwd | 固定为 main dir（默认 `~/.yue/main`，可配置），通过 delegate_agent 操作其它项目 |
...
判定方案：**cwd 匹配 main agent 工作目录**。main agent 的 cwd 固定为其工作目录（默认 `~/.yue/main`），不指向其它项目。工作目录可通过 `--main-dir <path>` 配置。

- `yue --main`：启动时进入 main agent 模式（前台交互），cwd = main dir
- `yue --main --daemon`：常驻后台模式
...
- `{soulPath}`（默认 `~/.yue/main/SOUL.md`）→ `getSoulFile()`
- `{memoryDir}/*.md`（默认 `~/.yue/main/memory/`）→ `getLongTermMemory()`
...
不引入专用 `memory` 工具。`~/.yue/main/memory/*.md` 就是普通 markdown 文件，agent 用现有的 `read` / `write` / `edit` / `grep` 直接操作：

- 读：`read ~/.yue/main/memory/user-profile.md`
- 写：`write ~/.yue/main/memory/<topic>.md ...`
- 搜索：`grep -r "keyword" ~/.yue/main/memory/`
...
第一次 `yue --main` 时 `~/.yue/main/` 不存在。初始化逻辑：

1. 创建 `~/.yue/main/` + `~/.yue/main/memory/` 目录
2. 写入默认 `SOUL.md` 模板（包含 Identity/Values/Voice 占位段，提示用户自定义）
3. 创建 `memory/_index.md`（空索引）+ `memory/user-profile.md`（空模板）
4. 输出提示："Main agent 已初始化，编辑 ~/.yue/main/SOUL.md 定义你的人格"
...
- **Desktop 多 Sidecar**（`apps/desktop/src/bridge.rs`）：`BridgeState` 已实现按 cwd 管理多个 `yue --mode rpc` 子进程的编排器，事件按 cwd 标签路由

### 8.2 分阶段方案

#### Phase 1：Shell 委派（零代码改动）

在 `isMainAgent` guidelines 中告知 main agent 可以用 `cli` 工具直接调用 yue CLI：

```bash
yue --cwd /path/to/project "fix the auth bug in login.ts"
```
...
- **Auth 共享**：`RpcClient` spawn 的子进程继承 `process.env`，默认读取 `~/.yue/agent/auth.json`，与 main agent 共享同一套凭证。
  > **⚠️ agentDir 对齐**：如果 main agent 使用了非默认 `agentDir`（如 `--agent-dir /custom/path`），子进程仍走默认 `getAgentDir()`，会读错 auth 文件。此时需在 `RpcClientOptions.env` 中显式传递 `YUE_AGENT_DIR` 或在子进程 args 中加 `--agent-dir`。
...
`~/.yue/main/memory/projects.md` 作为 main agent 的"项目注册表"，维护跨项目的工作上下文：
...
2. **首次运行初始化**：创建 `~/.yue/main/` 目录结构 + 默认 SOUL.md 模板 + 单实例 lockfile
